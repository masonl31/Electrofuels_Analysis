...
*indført bypass på BPR 20151027, søg %bypass%
*Filer: B.gms, balopt.opt, gdataset, gdata
...
$ifi     exist '../addons/_hooks/addonsopt.opt'  $include         '../addons/_hooks/addonsopt.opt';
$ifi not exist '../addons/_hooks/addonsopt.opt'  $include '../../base/addons/_hooks/addonsopt.opt';
...
$if     EXIST '../addons/_hooks/setdeclare.inc' $INCLUDE      '../addons/_hooks/setdeclare.inc';
$if not EXIST '../addons/_hooks/setdeclare.inc' $INCLUDE '../../base/addons/_hooks/setdeclare.inc';

$if     EXIST '../addons/_hooks/setdefine.inc' $INCLUDE      '../addons/_hooks/setdefine.inc';
$if not EXIST '../addons/_hooks/setdefine.inc' $INCLUDE '../../base/addons/_hooks/setdefine.inc';


*$ifi %COMBTECH%==yes  $if     EXIST '../data/ggcomb.inc' $INCLUDE         '../data/ggcomb.inc';    erstattet af _hook above
*$ifi %COMBTECH%==yes  $if not EXIST '../data/ggcomb.inc' $INCLUDE '../../base/data/ggcomb.inc';    erstattet af _hook
...
PARAMETER RAMPUP(FFF)    'Ramp-up constraint of technologies (% of the nominal capacity)' %semislash%
$if     EXIST '../data/RAMPUP.inc' $INCLUDE         '../data/RAMPUP.inc';
$if not EXIST '../data/RAMPUP.inc' $INCLUDE '../../base/data/RAMPUP.inc';
%semislash%

PARAMETER RAMPDOWN(FFF)    'Ramp-down constraint of technologies (% of the nominal capacity)' %semislash%
$if     EXIST '../data/RAMPDOWN.inc' $INCLUDE         '../data/RAMPDOWN.inc';
$if not EXIST '../data/RAMPDOWN.inc' $INCLUDE '../../base/data/RAMPDOWN.inc';
%semislash%
...
$if     EXIST '../addons/_hooks/isetdeclare.inc' $INCLUDE      '../addons/_hooks/isetdeclare.inc';
$if not EXIST '../addons/_hooks/isetdeclare.inc' $INCLUDE '../../base/addons/_hooks/isetdeclare.inc';

$if     EXIST '../addons/_hooks/isetdefine.inc' $INCLUDE      '../addons/_hooks/isetdefine.inc';
$if not EXIST '../addons/_hooks/isetdefine.inc' $INCLUDE '../../base/addons/_hooks/isetdefine.inc';

$if     EXIST '../addons/_hooks/isetdecdef.inc' $INCLUDE      '../addons/_hooks/isetdecdef.inc';
$if not EXIST '../addons/_hooks/isetdecdef.inc' $INCLUDE '../../base/addons/_hooks/isetdecdef.inc';


*$ifi %COMBTECH%==yes   IGCOMB1(G) =no;
*$ifi %COMBTECH%==yes   IGCOMB2(G) =no;
*$ifi %COMBTECH%==yes   $include '../../base/addons/combtech/combGassign.inc';  irrelevant now, icluded in  _hooks\isetdecdef.inc above
...
*$ifi %COMBTECH%==yes IAGK(IA,IGCOMB2)$SUM(IGCOMB1$(IAGK(IA,IGCOMB1) and GGCOMB(IGCOMB1,IGCOMB2)),1)=yes;    Idea now: handle combtechs as any tech in all non-tech sets,  parameters etc.
...
$ifi %GMAXF_DOL%==CCCRRRAAA_FFF         PARAMETER GMAXFCOMB(CCCRRRAAA,IF1,IF2)     'Maximum fuel use of two combined fuels (GJ) per year';
$ifi %GMAXF_DOL%==YYY_CCCRRRAAA_FFF     PARAMETER GMAXFCOMB(YYY,CCCRRRAAA,IF1,IF2) 'Maximum fuel use of two combined fuels (GJ) per year';
...
PARAMETER XLIFETIME(IRRRE,IRRRI)       'Technical lifetime of new transmission capacity (years)';
...
$ifi %GMAXF_DOL%==CCCRRRAAA_FFF      PARAMETER GMAXFCOMB(CCCRRRAAA,IF1,IF2)             'Maximum use of two combined fuels (GJ) per year' %semislash%
$ifi %GMAXF_DOL%==YYY_CCCRRRAAA_FFF  PARAMETER GMAXFCOMB(YYY,CCCRRRAAA,IF1,IF2)         'Maximum use of two combined fuels (GJ) per year' %semislash%
$if     EXIST '../data/GMAXFCOMB.inc' $INCLUDE      '../data/GMAXFCOMB.inc';
$if not EXIST '../data/GMAXFCOMB.inc' $INCLUDE '../../base/data/GMAXFCOMB.inc';
%semislash%;
...
PARAMETER XLIFETIME(IRRRE,IRRRI)  'Technical lifetime of new transmission capacity (years)'    %semislash%
$if     EXIST '../data/XLIFETIME.inc' $INCLUDE      '../data/XLIFETIME.inc';
$if not EXIST '../data/XLIFETIME.inc' $INCLUDE '../../base/data/XLIFETIME.inc';
%semislash%;
...
PARAMETER XMAXINV(IYALIAS,YYY,IRRRE,IRRRI)   'Max investment in transmission capacity between two regions for each simulated year(each 5th year)'  %semislash%
...
PARAMETER GVKXN(YYY,IRRRE,IRRRI) 'Investments in generation technology by BB2 up to and including year Y';
...
$if     EXIST '../data/GVKGN.inc' $INCLUDE      '../data/GVKGN.inc';
$if not EXIST '../data/GVKGN.inc' $INCLUDE '../../base/data/GVKGN.inc';

$if     EXIST '../data/GVKXN.inc' $INCLUDE      '../data/GVKXN.inc';
$if not EXIST '../data/GVKXN.inc' $INCLUDE '../../base/data/GVKXN.inc';
...
DH_VAR_T(IA,SSS,TTT)$(NOT (SUM(Y,DH(Y,IA)) GT 0.01)) = 0;
...
*$ifi %COMBTECH%==yes    IAGKN(IA,IGCOMB2)=NO;  why not??       OK, taget ud
...
PARAMETER  IGMAXF_Y(CCCRRRAAA,FFF)     'Maximum fuel use (GJ) per year',
           ...
           IGMAXFCOMB_Y(CCCRRRAAA,IF1,IF2) 'Maximum use of two combined fuels (GJ) per year';
...
PARAMETER IXMAXINV_Y(IYALIAS,IRRRE,IRRRI)   'Pair of regions that may get new transmission capacity contrained by an increased compared to the base year IYALIAS';

Parameter XKRATE_Y(YYY,IRRRE,IRRRI,SSS,TTT) 'Availability of transmission lines per year';
XKRATE_Y('2014','DK_E','DE_NE',S,T)=0.13;
XKRATE_Y('2014','DK_E','SE_S',S,T)=0.72;
XKRATE_Y('2014','DK_W','NO_S',S,T)=0.86;
XKRATE_Y('2014','DE_NE','SE_S',S,T)=0.45;
XKRATE_Y('2014','DE_NE','DK_E',S,T)=0.13;
XKRATE_Y('2014','SE_S','DK_E',S,T)=0.72;
XKRATE_Y('2014','NO_S','DK_W',S,T)=0.86;
XKRATE_Y('2014','SE_S','DE_NE',S,T)=0.45;

XKRATE_Y('2020','DK_E','DE_NE',S,T)=0.20;
XKRATE_Y('2020','DK_E','SE_S',S,T)=0.75;
XKRATE_Y('2020','DK_W','NO_S',S,T)=0.86;
XKRATE_Y('2020','DE_NE','SE_S',S,T)=0.50;
XKRATE_Y('2020','DE_NE','DK_E',S,T)=0.20;
XKRATE_Y('2020','SE_S','DK_E',S,T)=0.75;
XKRATE_Y('2020','NO_S','DK_W',S,T)=0.86;
XKRATE_Y('2020','SE_S','DE_NE',S,T)=0.50;

XKRATE_Y('2025','DK_E','DE_NE',S,T)=0.65;
XKRATE_Y('2025','DK_E','SE_S',S,T)=0.84;
XKRATE_Y('2025','DK_W','NO_S',S,T)=0.89;
XKRATE_Y('2025','DE_NE','SE_S',S,T)=0.76;
XKRATE_Y('2025','DE_NE','DK_E',S,T)=0.65;
XKRATE_Y('2025','SE_S','DK_E',S,T)=0.84;
XKRATE_Y('2025','NO_S','DK_W',S,T)=0.89;
XKRATE_Y('2025','SE_S','DE_NE',S,T)=0.76;

LOOP(Y,XKRATE(IRE,IRI,S,T)$(XKRATE_Y(Y,IRE,IRI,S,T))=XKRATE_Y(Y,IRE,IRI,S,T););
...
$if     EXIST '../data/TIMELENGHT.inc' $INCLUDE      '../data/TIMELENGHT.inc';
$if not EXIST '../data/TIMELENGHT.inc' $INCLUDE '../../base/data/TIMELENGHT.inc';
%semislash%;

PARAMETER CYCLES(TTT);
CYCLES(T)=CARD(TTT)/(CARD(T)*TIMELENGHT(T));

SET RINDUS(RRR) /DK_E_Industry, DK_W_Industry/;
SET AINDHEAT(AAA) /DK_W_IndETOH, DK_E_IndETOH/;

PARAMETER FLIMCONS(YYY,C,FFF)
/
2035 . DENMARK . NAT_GAS  0.05
2040 . DENMARK . NAT_GAS  0.05
2045 . DENMARK . NAT_GAS  0.05
2050 . DENMARK . NAT_GAS  0.05
/;

PARAMETER IFLIMCONS_Y(C,FFF);

*FOR the year 2050:Move this out so that it exists if GEQF or GMAXF are equal to 0 or to epsilon, in order to make it more general


AGKN(AAA,G)$((NOT IGHSTOALL(G)) AND (NOT IGESTOALL(G)) AND (NOT IGH2STO(G)) )=NO;
AGKN(AAA,G)=AGKN_INITIAL(AAA,G)+AGKN(AAA,G);
AGKN(AAA,IGCOMB2)$SUM(IGCOMB1$GGCOMB(IGCOMB1,IGCOMB2),AGKN(AAA,IGCOMB1))=YES;

*Move this out to the AGKN file??



$if     EXIST '../addons/_hooks/parameters.inc' $INCLUDE      '../addons/_hooks/parameters.inc';
$if not EXIST '../addons/_hooks/parameters.inc' $INCLUDE '../../base/addons/_hooks/parameters.inc';

$if     EXIST '../addons/_hooks/parameters_Bal_update.inc' $INCLUDE      '../addons/_hooks/parameters_Bal_update.inc';
$if not EXIST '../addons/_hooks/parameters_Bal_update.inc' $INCLUDE '../../base/addons/_hooks/parameters_Bal_update.inc';
...
SET IGWNDON(G)'Onshore Wind technologies' %semislash%
$if     EXIST '../data/IGWNDON.inc' $INCLUDE         '../data/IGWNDON.inc';
$if not EXIST '../data/IGWNDON.inc' $INCLUDE '../../base/data/IGWNDON.inc';
%semislash%

Parameter INSCAPWINDON(YYY,CCCRRRAAA)'Maximum onshore wind installed capacity in every country in every year' %semislash%
...
Parameter INSCAPWINDON_Y(CCCRRRAAA) 'Maximum onshore wind installed capacity for the year of simulation' %semislash%
INSCAPWINDON_Y(CCCRRRAAA)=SMAX(Y,INSCAPWINDON(Y,CCCRRRAAA));
%semislash%

SET IGWNDOFF(G)'Offshore Wind technologies' %semislash%
$if     EXIST '../data/IGWNDOFF.inc' $INCLUDE         '../data/IGWNDOFF.inc';
$if not EXIST '../data/IGWNDOFF.inc' $INCLUDE '../../base/data/IGWNDOFF.inc';
%semislash%

Parameter INSCAPWINDOFF(YYY,C)'Maximum offshore wind installed capacity in every country in every year' %semislash%
...
Parameter INSCAPWINDOFF_Y(C) 'Maximum offshore wind installed capacity for the year of simulation' %semislash%
INSCAPWINDOFF_Y(C)=SMAX(Y,INSCAPWINDOFF(Y,C));
%semislash%

SET HEATPUMP_ST(G) 'Standard Heat Pumps'  %semislash%
$if     EXIST '../data/HEATPUMP_ST.inc' $INCLUDE         '../data/HEATPUMP_ST.inc';
$if not EXIST '../data/HEATPUMP_ST.inc' $INCLUDE '../../base/data/HEATPUMP_ST.inc';
%semislash%

Parameter INSCAPHEATPUMP(YYY,AAA)'Maximum offshore wind installed capacity in every country in every year' %semislash%
$if     EXIST '../data/INSCAPHEATPUMP.inc' $INCLUDE         '../data/INSCAPHEATPUMP.inc';
$if not EXIST '../data/INSCAPHEATPUMP.inc' $INCLUDE '../../base/data/INSCAPHEATPUMP.inc';
%semislash%

Parameter INSCAPHEATPUMP_Y(AAA) 'Maximum installed capacity of large DH' %semislash%
INSCAPHEATPUMP_Y(AAA)=SMAX(Y,INSCAPHEATPUMP(Y,AAA));
%semislash%
...
POSITIVE VARIABLE VQGEQRF_S(RRR,FFF,S,IPLUSMINUS)

*Amalia - introduction of possibility to cool down some heat
POSITIVE VARIABLE VCOOLDOWN(AAA,S,T) 'Possibility to cool down some heat (MW)'
POSITIVE VARIABLE ECHECKING(CCC,S,T)
...
$if     EXIST '../addons/_hooks/defvariables.inc' $INCLUDE      '../addons/_hooks/defvariables.inc';
$if not EXIST '../addons/_hooks/defvariables.inc' $INCLUDE '../../base/addons/_hooks/defvariables.inc';
...
EQUATIONS
   ...
   QGMAXCFCOMB(C,IF1,IF2) 'Maximum usage of two fuels per country constraint (MWh)'
   ...
   QXMAXINV(IYALIAS,IRRRE,IRRRI)  'Limit of new transmission capacity (MW)'
   ...
   QGFRAMPUP(AAA,G,S,T)   'Maximum ramp-up between two consequentional times in existing units (MW)'
   QGFRAMPDOWN(AAA,G,S,T) 'Maximum ramp-down between two consequentional times in existing units (MW)'
   QGFNRAMPUP(AAA,G,S,T)  'Maximum ramp-up between two consequentional times in new units (MW)'
   QGFNRAMPDOWN(AAA,G,S,T)'Maximum ramp-down between two consequentional times in new units (MW)'

* Equations to add some extra constaints
   QWINDON_C(C)   'Onshore wind maximum installed capacity of the scenario'
   QWINDON_IR(RRR) 'Onshore wind maximum installed capacity of the scenario'
   QWINDOFF(C)    'Offshore wind maximum installed capacity of the scenario'
   ...
   QFOSSILFUELCONS(C,FFF) 'Equation to limit the amount of fossil fuel consumption subjet to the total demand in the country'
   QHEATPUMPLARGEDH(AAA)  'Heat Pumps in Large Areas'
...
$if     EXIST '../addons/_hooks/qobj.inc' $INCLUDE      '../addons/_hooks/qobj.inc';
$if not EXIST '../addons/_hooks/qobj.inc' $INCLUDE '../../base/addons/_hooks/qobj.inc';
...
* Operation and maintainance cost:
   ...
   + SUM(IAGK_Y(IA,IGNOTETOH(IGE)), GOMVCOST(IA,IGNOTETOH) * SUM((IS3,T), IHOURSINST(IS3,T)*
     VGE_T(IA,IGNOTETOH,IS3,T)))

   + SUM(IAGK_Y(IA,IGH), GOMVCOST(IA,IGH) * SUM((IS3,T), IHOURSINST(IS3,T)*
     GDATA(IGH,'GDCV')*(VGH_T(IA,IGH,IS3,T))))

   + SUM(IAGKN(IA,IGNOTETOH(IGE)), GOMVCOST(IA,IGNOTETOH) * SUM((IS3,T), IHOURSINST(IS3,T)*
     VGEN_T(IA,IGNOTETOH,IS3,T)))

   + SUM(IAGKN(IA,IGH), GOMVCOST(IA,IGH) * SUM((IS3,T), IHOURSINST(IS3,T)*
     GDATA(IGH,'GDCV')*(VGHN_T(IA,IGH,IS3,T))))
...
$ifi %YIELDREQUIREMENT%==yes YIELDREQ(G)*
     ((SUM(C$ICA(C,IA),ANNUITYC(C)))/(1-(1+(SUM(C$ICA(C,IA),ANNUITYC(C))))**(-GDATA(G,'GDLIFETIME'))))
      ))
...
*      (IOF05*(SUM(C$CCCRRR(C,IRE),ANNUITYC(C))+SUM(C$CCCRRR(C,IRI),ANNUITYC(C)))))
     (IOF05*(((SUM(C$CCCRRR(C,IRE),ANNUITYC(C)))/(1-(1+(SUM(C$CCCRRR(C,IRE),ANNUITYC(C))))**(-XLIFETIME(IRE,IRI))))+
     ((SUM(C$CCCRRR(C,IRI),ANNUITYC(C)))/(1-(1+(SUM(C$CCCRRR(C,IRI),ANNUITYC(C))))**(-XLIFETIME(IRE,IRI)))))))
...
$ifi %BB2%==yes    +SUM((IA,IS3)$SUM(IGHYRS,IAGK_Y(IA,IGHYRS)),(VQHYRSSEQ(IA,IS3,'IMINUS')+VQHYRSSEQ(IA,IS3,'IPLUS')))
               ...
               +SUM((IR,FFF,IS3)$IGEQF_Y_S(IR,FFF,IS3), VQGEQRF_S(IR,FFF,IS3,'IPLUS')+VQGEQRF_S(IR,FFF,IS3,'IMINUS'))
...
$if     EXIST '../addons/_hooks/qeeq.inc' $INCLUDE      '../addons/_hooks/qeeq.inc';
$if not EXIST '../addons/_hooks/qeeq.inc' $INCLUDE '../../base/addons/_hooks/qeeq.inc';
...
$if     EXIST '../addons/_hooks/qheq.inc' $INCLUDE      '../addons/_hooks/qheq.inc';
$if not EXIST '../addons/_hooks/qheq.inc' $INCLUDE '../../base/addons/_hooks/qheq.inc';
...
QESTOVOLT(IA,S,T)$(SUM(IGESTO, IAGK_Y(IA,IGESTO)+IAGKN(IA,IGESTO)) AND IS3(S))..
  ...
  + (IHOURSINST(S,T)*(CARD(S)/CARD(SSS))*(1/CYCLES(T)))*(VESTOLOADT(IA,S,T)
...
QESTOVOLTS(IA,S,T)$(SUM(IGESTOS, IAGK_Y(IA,IGESTOS)+IAGKN(IA,IGESTOS)))..
    VESTOVOLTS(IA,S,T)
  + (IHOURSINST(S,T)*(CARD(S)/CARD(SSS))*(1/CYCLES(T)))* (VESTOLOADTS(IA,S,T)
...
QHSTOVOLT(IA,S,T)$(SUM(IGHSTO, IAGK_Y(IA,IGHSTO)+IAGKN(IA,IGHSTO)))..
    VHSTOVOLT(IA,S,T)
  + (IHOURSINST(S,T)*(CARD(S)/CARD(SSS))*(1/CYCLES(T)))* (VHSTOLOADT(IA,S,T)
...
QHSTOVOLTS(IA,S,T)$(SUM(IGHSTOS, IAGK_Y(IA,IGHSTOS)+IAGKN(IA,IGHSTOS)))..
    VHSTOVOLTS(IA,S,T)
  + (IHOURSINST(S,T)*(CARD(S)/CARD(SSS))*(1/CYCLES(T)))* (VHSTOLOADTS(IA,S,T)
...
QGEQRF_S(IR,FFF,IS3)$IGEQF_Y_S(IR,FFF,IS3)..
    IGEQF_Y_S(IR,FFF,IS3)+ VQGEQRF_S(IR,FFF,IS3,'IPLUS') - VQGEQRF_S(IR,FFF,IS3,'IMINUS')
   =L=
    SUM(IA$RRRAAA(IR,IA),
    SUM(G$(IAGK_Y(IA,G) AND (GDATA(G,'GDFUEL') EQ FDATA(FFF,'FDACRONYM'))), IOF3P6 * SUM(T, IHOURSINST(IS3,T) * VGF_T(IA,G,IS3,T)))
    +SUM(G$(IAGKN(IA,G)  AND (GDATA(G,'GDFUEL') EQ FDATA(FFF,'FDACRONYM'))), IOF3P6 * SUM(T, IHOURSINST(IS3,T) * VGFN_T(IA,G,IS3,T)))
...
QGMAXCFCOMB(C,IF1,IF2)$IGMAXFCOMB_Y(C,IF1,IF2)..
    IGMAXFCOMB_Y(C,IF1,IF2)
             =G=
    SUM(IA$ICA(C,IA),
     SUM(G$(IAGK_Y(IA,G) AND ((GDATA(G,'GDFUEL') EQ FDATA(IF1,'FDACRONYM')) OR (GDATA(G,'GDFUEL') EQ FDATA(IF2,'FDACRONYM')))), IOF3P6 * SUM((IS3,T), IHOURSINST(IS3,T) * VGF_T(IA,G,IS3,T)))
    +SUM(G$(IAGKN(IA,G)  AND ((GDATA(G,'GDFUEL') EQ FDATA(IF1,'FDACRONYM')) OR (GDATA(G,'GDFUEL') EQ FDATA(IF2,'FDACRONYM')))), IOF3P6 * SUM((IS3,T), IHOURSINST(IS3,T) * VGFN_T(IA,G,IS3,T)))
    );
...
QGFRAMPUP(IAGK_Y(IA,G),IS3,T)$(SUM(FFF$IGF(G,FFF),RAMPUP(FFF)) AND (NOT (ORD(T) EQ CARD(T)))) ..
VGF_T(IA,G,IS3,T+1)-VGF_T(IA,G,IS3,T)=L=SUM(FFF$IGF(G,FFF),RAMPUP(FFF)*TIMELENGHT(T))*IGKFX_Y(IA,G)*(
((1+GDATA(G,'GDCB'))/(GDATA(G,'GDFE')*GDATA(G,'GDCB')))$IGBPR(G)+
(1/(GDATA(G,'GDFE')))$(IGH(G) OR IGCND(G) OR IGEXT(G)));


QGFRAMPDOWN(IAGK_Y(IA,G),IS3,T)$(SUM(FFF$IGF(G,FFF),RAMPDOWN(FFF))AND (NOT (ORD(T) EQ CARD(T))))..
VGF_T(IA,G,IS3,T)-VGF_T(IA,G,IS3,T+1)=L=SUM(FFF$IGF(G,FFF),RAMPDOWN(FFF)*TIMELENGHT(T))*IGKFX_Y(IA,G)*(
((1+GDATA(G,'GDCB'))/(GDATA(G,'GDFE')*GDATA(G,'GDCB')))$IGBPR(G)+
(1/(GDATA(G,'GDFE')))$(IGH(G) OR IGCND(G) OR IGEXT(G)));

QGFNRAMPUP(IAGKN(IA,G),IS3,T)$(SUM(FFF$IGF(G,FFF),RAMPUP(FFF))AND (NOT (ORD(T) EQ CARD(T))))..
VGFN_T(IA,G,IS3,T+1)-VGFN_T(IA,G,IS3,T)=L=SUM(FFF$IGF(G,FFF),RAMPUP(FFF)*TIMELENGHT(T))*VGKN(IA,G)*(
((1+GDATA(G,'GDCB'))/(GDATA(G,'GDFE')*GDATA(G,'GDCB')))$IGBPR(G)+
(1/(GDATA(G,'GDFE')))$(IGH(G) OR IGCND(G) OR IGEXT(G)));

QGFNRAMPDOWN(IAGKN(IA,G),IS3,T)$(SUM(FFF$IGF(G,FFF),RAMPDOWN(FFF))AND (NOT (ORD(T) EQ CARD(T))))..
VGFN_T(IA,G,IS3,T)-VGFN_T(IA,G,IS3,T+1)=L=SUM(FFF$IGF(G,FFF),RAMPDOWN(FFF)*TIMELENGHT(T))*VGKN(IA,G)*(
((1+GDATA(G,'GDCB'))/(GDATA(G,'GDFE')*GDATA(G,'GDCB')))$IGBPR(G)+
(1/(GDATA(G,'GDFE')))$(IGH(G) OR IGCND(G) OR IGEXT(G)));
...
QXK(IRE,IRI,IS3,T)$((IXKINI_Y(IRE,IRI) OR IXKN(IRI,IRE) OR IXKN(IRE,IRI)) AND (IXKINI_Y(IRE,IRI) NE INF) )..
     ...
     + VXKN(IRE,IRI)$(IXKN(IRE,IRI) AND (SUM(IYALIAS,IXMAXINV_Y(IYALIAS,IRE,IRI))))+ VXKN(IRI,IRE)$(IXKN(IRI,IRE) AND (SUM(IYALIAS,IXMAXINV_Y(IYALIAS,IRI,IRE))))
     )
     + VQXK(IRE,IRI,IS3,T,'IPLUS')-VQXK(IRE,IRI,IS3,T,'IMINUS')
     =G=  VX_T(IRE,IRI,IS3,T);


QXMAXINV(IYALIAS,IRE,IRI)$(IXKN(IRE,IRI) AND (IXMAXINV_Y(IYALIAS,IRE,IRI)))..
         IXMAXINV_Y(IYALIAS,IRE,IRI)+XKINI(IYALIAS,IRE,IRI)
=G=
         VXKN(IRE,IRI)+IXKINI_Y(IRE,IRI)+IXKVACCTOY(IRE,IRI);
...
QSOLE(C)$INSCAPSOLE_Y(C)..
   INSCAPSOLE_Y(C)=G=SUM(IA$ICA(C,IA),(SUM(IGSOLE,VGKN(IA,IGSOLE)$(IAGKN(IA,IGSOLE))+IGKFXYMAX(IA,IGSOLE)+IGKVACCTOY(IA,IGSOLE))));

QSOLH(C)$INSCAPSOLH_Y(C)..
   INSCAPSOLH_Y(C)=G=SUM(IA$ICA(C,IA),(SUM(IGSOLH,VGKN(IA,IGSOLH)$(IAGKN(IA,IGSOLH))+IGKFXYMAX(IA,IGSOLH)+IGKVACCTOY(IA,IGSOLH))));

QWINDON_C(C)$INSCAPWINDON_Y(C)..
   INSCAPWINDON_Y(C)=G=SUM(IA$ICA(C,IA),(SUM(IGWNDON,VGKN(IA,IGWNDON)$(IAGKN(IA,IGWNDON))+IGKFXYMAX(IA,IGWNDON)+IGKVACCTOY(IA,IGWNDON))));

QWINDON_IR(IR)$INSCAPWINDON_Y(IR)..
   INSCAPWINDON_Y(IR)=G=SUM(IA$RRRAAA(IR,IA),(SUM(IGWNDON,VGKN(IA,IGWNDON)$(IAGKN(IA,IGWNDON))+IGKFXYMAX(IA,IGWNDON)+IGKVACCTOY(IA,IGWNDON))));

QWINDOFF(C)$INSCAPWINDOFF_Y(C)..
   INSCAPWINDOFF_Y(C)=G=SUM(IA$ICA(C,IA),(SUM(IGWNDOFF,VGKN(IA,IGWNDOFF)$(IAGKN(IA,IGWNDOFF))+IGKFXYMAX(IA,IGWNDOFF)+IGKVACCTOY(IA,IGWNDOFF))));


QHEATPUMPLARGEDH(IA)$INSCAPHEATPUMP_Y(IA)..
   SUM(G$HEATPUMP_ST(G),IGKFXYMAX(IA,G)+ VGKN(IA,G)$IAGKN(IA,G))=L=INSCAPHEATPUMP_Y(IA);
...
$if     EXIST '../addons/_hooks/eqndeclare.inc' $INCLUDE      '../addons/_hooks/eqndeclare.inc';
$if not EXIST '../addons/_hooks/eqndeclare.inc' $INCLUDE '../../base/addons/_hooks/eqndeclare.inc';

$if     EXIST '../addons/_hooks/eqndefine.inc' $INCLUDE      '../addons/_hooks/eqndefine.inc';
$if not EXIST '../addons/_hooks/eqndefine.inc' $INCLUDE '../../base/addons/_hooks/eqndefine.inc';

$if     EXIST '../addons/_hooks/eqndecdef.inc' $INCLUDE      '../addons/_hooks/eqndecdef.inc';
$if not EXIST '../addons/_hooks/eqndecdef.inc' $INCLUDE '../../base/addons/_hooks/eqndecdef.inc';


* These add-on equations pertain to district heating.
$ifi %FV%==yes $include '../../base/addons/Fjernvarme/eq_fv.inc';

* These add-on equations pertain to combination technologies.                     !! TODO
*$ifi %COMBTECH%==yes $include '../../base/addons/combtech/combEQ.inc';   included in _hook above
...
QSELFSUFFICIENCY('DENMARK')..
SUM((IS3,T),IHOURSINST(IS3,T) * (SUM((IR,IRI)$(CCCRRR('DENMARK',IR) AND (NOT CCCRRR('DENMARK',IRI))AND (IXKINI_Y(IR,IRI) OR IXKN(IRI,IR) OR IXKN(IR,IRI)) AND (IXKINI_Y(IR,IRI) NE INF)), VX_T(IR,IRI,IS3,T))))
- SUM((IA,FFF)$(ICA('DENMARK',IA) AND IFLIMCONS_Y('DENMARK',FFF)),
  SUM(G$(IAGK_Y(IA,G) AND (GDATA(G,'GDFUEL') EQ FDATA(FFF,'FDACRONYM'))), SUM((IS3,T), IHOURSINST(IS3,T) * VGF_T(IA,G,IS3,T)))
  +SUM(G$(IAGKN(IA,G)  AND (GDATA(G,'GDFUEL') EQ FDATA(FFF,'FDACRONYM'))), SUM((IS3,T), IHOURSINST(IS3,T) * VGFN_T(IA,G,IS3,T))))
=G=
SUM((IS3,T),IHOURSINST(IS3,T)*(SUM((IR,IRE)$(CCCRRR('DENMARK',IR) AND (NOT CCCRRR('DENMARK',IRE))AND (IXKINI_Y(IRE,IR) OR IXKN(IR,IRE) OR IXKN(IRE,IR)) AND (IXKINI_Y(IRE,IR) NE INF)), VX_T(IRE,IR,IS3,T))));


QFOSSILFUELCONS(C,FFF)$(IFLIMCONS_Y(C,FFF))..
    SUM(IA$ICA(C,IA),
     SUM(G$(IAGK_Y(IA,G) AND (GDATA(G,'GDFUEL') EQ FDATA(FFF,'FDACRONYM'))), SUM((IS3,T), IHOURSINST(IS3,T) * VGF_T(IA,G,IS3,T)))
    +SUM(G$(IAGKN(IA,G)  AND (GDATA(G,'GDFUEL') EQ FDATA(FFF,'FDACRONYM'))), SUM((IS3,T), IHOURSINST(IS3,T) * VGFN_T(IA,G,IS3,T))))
     =L=
    IFLIMCONS_Y(C,FFF)*(SUM((IR,IS3,T)$CCCRRR(C,IR),IHOURSINST(IS3,T)*(
    IDE_T_Y(IR,IS3,T)$(NOT (RETRANS(IR) OR RINDUS(IR)))
    +SUM(RETRANS$IXKINI_Y(IR,RETRANS),IDE_T_Y(RETRANS,IS3,T))
    +SUM(IAGK_Y(IA,IGE)$((RRRAAA(IR,IA) AND (AINDHEAT(IA))) AND IGETOH(IGE)), VGE_T(IA,IGE,IS3,T))
    +SUM(IAGKN(IA,IGE)$((RRRAAA(IR,IA) AND (AINDHEAT(IA))) AND IGETOH(IGE)), VGEN_T(IA,IGE,IS3,T))
    +SUM(RINDUS$IXKINI_Y(IR,RINDUS),IDE_T_Y(RINDUS,IS3,T))
    +SUM(IAGK_Y(IA,IGE)$((RRRAAA(IR,IA) AND (NOT AINDHEAT(IA))) AND IGETOH(IGE)), VGE_T(IA,IGE,IS3,T))
    +SUM(IAGKN(IA,IGE)$((RRRAAA(IR,IA) AND (NOT AINDHEAT(IA))) AND IGETOH(IGE)), VGEN_T(IA,IGE,IS3,T))
     )));
...
$if     EXIST '../addons/_hooks/balbase1.inc' $INCLUDE      '../addons/_hooks/balbase1.inc';
$if not EXIST '../addons/_hooks/balbase1.inc' $INCLUDE '../../base/addons/_hooks/balbase1.inc';


*$ifi %COMBTECH%==yes $include '../../base/addons/combtech/combbb1.inc';  replaced by _hook
...
$ifi %X3VfxQ%==yes              QX3VBAL
$if     EXIST '../addons/_hooks/balbase2.inc' $INCLUDE      '../addons/_hooks/balbase2.inc';
$if not EXIST '../addons/_hooks/balbase2.inc' $INCLUDE '../../base/addons/_hooks/balbase2.inc';
...
$if     EXIST '../addons/_hooks/balbase3.inc' $INCLUDE      '../addons/_hooks/balbase3.inc';
$if not EXIST '../addons/_hooks/balbase3.inc' $INCLUDE '../../base/addons/_hooks/balbase3.inc';
...
$if     EXIST '../addons/_hooks/error2.inc' $INCLUDE      '../addons/_hooks/error2.inc';
$if not EXIST '../addons/_hooks/error2.inc' $INCLUDE '../../base/addons/_hooks/error2.inc';


*$ifi %COMBTECH%==yes  $INCLUDE  '../../base/addons/combtech/comberr2.inc';     replaced by above _hook
...
* <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
$ifi     %useGUSS%==yes $include "gussdict.inc";
* <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

*$if EXIST 'bb123.sim' $INCLUDE 'bb123.sim';


*$ifi     %useGUSS%==yes SOLVE BALBASE1 USING LP MINIMIZING VOBJ Scenario BB1gussdict;

$ifi     %useGUSS%==yes $goto endofmodel
...
$INCLUDE      '../output/inputout/OutputFile.inc';



execute_unload "all.gdx";

$ifi not %useGUSS%==yes  $goto endGuss99
execute_unload "scenout.gdx",OPTIONSGUSS, SOLHEADERS, SOLSTATUS_SCEN, scenariosUsed_E, IDE_T_Y_SCEN, BB1gussdict, VGE_TdotL_SCEN, VGF_TdotL_SCEN, QEEQdotM_SCEN;
OPTION         IDE_T_Y_SCEN_VALUE:0:2:2 , IDE_T_Y_SCEN_CHECK:0:2:2 , scen_Delta_E_init:0:2:2,scen_Delta_E:0:2:2, IDE_T_Y_SCEN:0:2:2, IDE_T_Y:0:2:1;
DISPLAY        "Checking IDE_T_Y_SCEN_CHECK in SCENARO_E.INC", scenariosUsed_E, scen_Delta_E_init, scen_Delta_E, IDE_T_Y, IDE_T_Y_SCEN /*,IDE_T_Y_SCEN_VALUE, IDE_T_Y_SCEN_CHECK*/;
execute_unload "scenresult.gdx" , OPTIONSGUSS, SOLSTATUS_SCEN /*, SolHeaders*/,ScenariosUsed_E, BB1gussdict, IDE_T_Y_SCEN, VGE_TdotL_SCEN , QEEQdotM_SCEN, VGF_TdotL_SCEN;
OPTION         IDE_T_Y_SCEN_VALUE:0:2:2 , IDE_T_Y_SCEN_CHECK:0:2:2 , scen_Delta_E_init:0:2:2,scen_Delta_E:0:2:2, IDE_T_Y_SCEN:0:2:2, IDE_T_Y:0:2:1;
display        OPTIONSGUSS, SOLSTATUS_SCEN, SolHeaders, IDE_T_Y_SCEN, VGE_TdotL_SCEN , VGF_TdotL_SCEN, QEEQdotM_SCEN;
$label endGuss99
